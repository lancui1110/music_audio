    ...
    function getXMLHttpData(){
        if(xmlhttp.readyState === 4){
            if(xmlhttp.status === 0 || xmlhttp.status === 200){
                ...
                console.log(JSON.stringify(lrcTimeArray));
        ...
        if(dataTimeLine > 0 && dataTimeLine === parseInt(currenttime)){
            ...
            if(s != i){
                ...
                for(var j=0;j<domList.length;j++){
                    // 如果是原来有color的行，停止滚动文字
                    if (hasClass(domList[j], "color")) {
                      trigger(domList[j], 'mouseout');
                    }
                ...
                if(index > 0){
                    ...
                    trigger(domList[index], 'mouseover');
                }
...
function initLrcEvent () {
  console.log('initLrcEvent.....');
  var dnd = {
    // 初始化
    init: function () {
      var me = this;
      ...
      me.src = document.querySelector('#lrcBox');
      me.panelList = document.querySelector('#lrcMoveLine');

      me.src.addEventListener('mousedown', me.onMouseDown.bind(me), false);

      me.src.addEventListener('mousemove', me.onMouseMove.bind(me), false);

      me.src.addEventListener('mouseup', me.onMouseUp.bind(me), false);

      me.src.addEventListener('mouseout', me.onMouseOut.bind(me), false);

      // 鼠标放上行让字体滚动事件
      me.initLrcLineScrollEvent();
    },
    // 鼠标放上行让字体滚动
    initLrcLineScrollEvent: function () {
      var me = this;
      // 鼠标放上滚动
      document.addEventListener('mouseover', function (e) {
        var p = e.target
        if (p.className !== 'lrc-line') return;

        if (me.scrollLrclineTimer) clearInterval(me.scrollLrclineTimer);

        var scrollWidth = p.scrollWidth - me.src.width; // 滚动宽度
        if (scrollWidth <= 0) return;

        me.scrollLrclineTimer = setInterval(function(){
          if (p.scrollLeft >= scrollWidth) { // 滚到结束
            clearInterval(me.scrollLrclineTimer);
          }
          p.scrollLeft = p.scrollLeft + 1; // 滚动
        }, 15);
      ...
      document.addEventListener('mouseout', function (e) {
        if (e.target.className !== 'lrc-line') return;
        if (me.scrollLrclineTimer) clearInterval(me.scrollLrclineTimer);
    ...
    onMouseDown: function (e) {
      this.default.isClick = true
      this.default.startY = e.clientY
    },
    onMouseMove: function (e) {
      if (this.default.isClick) {
        var y = e.clientY - this.default.startY,
            top = this.src.style.marginTop || 0
        top = parseInt(top.replace('px', ''))

        this._setMarginTop(top + y)
        this.default.top = top + y
        this.default.startY = e.clientY
      }
    },
    onMouseUp: function (e) {
      console.log('onMouseUp...');
      this.default.isClick = false
      var top = this.default.top

      // 补整，移动到行中心
      offset = this.reset(top)
      this._setMarginTop(offset)
    },
    onMouseOut: function (e) {
      // this.default.isClick = false
    },
    _setMarginTop (offset) {
      var me = this
      var distance = offset || 0

      me.src.style.marginTop = `${offset}px`
    },

    // 补整，移动到行中心
    reset (offset) {
      var y = (offset) % 30
      console.log(`offset: ${offset} ${y} y:${offset - (30 + y)}`);
      if (y < 0) {
        offset = offset - (30 + y)
      } else if (y > 0) {
        offset = offset + (30 - y)
      ...
      // 获得移动到的行
      var curLine = this.getCurLrcLine(offset)
      // 设置当前时间为curLine的时间
      this.setCurrentTime(curLine)

      return offset
    },
    // 获得移动到的行
    getCurLrcLine(offset) {
      var index = Math.abs(offset / 30)
      var lines = this.src.childNodes
      return lines[index]
    },
    // 设置当前时间为curLine的时间
    setCurrentTime (curLine) {
      var mPlayer = document.getElementById("musicEngine");
      mPlayer.currentTime = curLine.attributes["data-timeLine"].nodeValue
    }
...
function trigger(el, type) {
  try{
    ...
    console.log('trigger error:' + type);
  }
